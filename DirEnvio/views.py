...
#ACTUALIZAR EL FORMULARIO DE DIRECCIONES
class UpdateDireccion(LoginRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    success_message = 'Enhorabuena Direccion Actualizada Correctamente'

    def get_success_url(self):
        return reverse ('direccion_envio')
    
#BORRAR UNA DIRECCION DEL LISTADO
class DeleteDireccion(LoginRequiredMixin, DeleteView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.get_object().default:
            messages.success('La direccion fue eliminada correctamente')
            return redirect ('direccion_envio')
...
#ASIGNAR FUNCIONALIDAD DE DEFAULT
@login_required(login_url='login')
